...
func NewCloudNamespaceHaUpdateCommand(cctx *CommandContext, parent *CloudNamespaceHaCommand) *CloudNamespaceHaUpdateCommand {
	...
	s.Command.Flags().BoolVar(&s.DisablePassivePollerForwarding, "disable-passive-poller-forwarding", false, "Set to true to disable passive poller forwarding for the namespace. Set to false to re-enable passive poller forwarding. Required.")
	_ = cobra.MarkFlagRequired(s.Command.Flags(), "disable-passive-poller-forwarding")
	s.ClientOptions.BuildFlags(s.Command.Flags())
	s.NamespaceOptions.BuildFlags(s.Command.Flags())
	s.AsyncOperationOptions.BuildFlags(s.Command.Flags())
	s.ResourceVersionOptions.BuildFlags(s.Command.Flags())
