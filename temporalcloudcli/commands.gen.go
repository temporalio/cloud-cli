...
func NewCloudUserSetNamespacePermissionsCommand(cctx *CommandContext, parent *CloudUserCommand) *CloudUserSetNamespacePermissionsCommand {
	...
	if hasHighlighting {
		s.Command.Long = "Set the namespace-level permissions for a Temporal Cloud user. Replaces all\nexisting namespace permissions with the ones provided.\n\nNamespace access format: 'namespace=permission' where permission is one of: admin, write, read.\n\nSpecify the user with either --user-id or --user-email (not both).\n\nExample:\n\n\x1b[1mcloud user set-namespace-permissions --user-id my-user-id \\\n  --namespace-access my-namespace.my-account=write \\\n  --namespace-access other-namespace.my-account=read\x1b[0m"
	} else {
	...
	s.Command.Flags().StringVar(&s.UserId, "user-id", "", "The ID of the user. Mutually exclusive with --user-email.")
	s.Command.Flags().StringVar(&s.UserEmail, "user-email", "", "The email address of the user. Mutually exclusive with --user-id.")
	s.Command.Flags().StringArrayVar(&s.NamespaceAccess, "namespace-access", nil, "Namespace access to set, in the format 'namespace=permission'. Permission must be one of: admin, write, read. Can be repeated. Replaces all existing namespace permissions. Required.")
