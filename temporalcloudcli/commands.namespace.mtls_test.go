...
func TestNamespaceMtlsGet(t *testing.T) {
	tests := []struct {
		name                    string
		cmd                     temporalcloudcli.CloudNamespaceMtlsGetCommand
		...
		{
			name: "Enabled",
			cmd:  temporalcloudcli.CloudNamespaceMtlsGetCommand{NamespaceOptions: temporalcloudcli.NamespaceOptions{Namespace: "my-ns.my-acct"}},
				...
				c.EXPECT().
					UpdateNamespace(mock.Anything, mock.MatchedBy(func(req *cloudservice.UpdateNamespaceRequest) bool {
						return req.Namespace == "my-ns.my-acct" &&
							...
							proto.Equal(req.Spec.MtlsAuth, &namespacev1.MtlsAuthSpec{Enabled: true})
					}), mock.Anything).
