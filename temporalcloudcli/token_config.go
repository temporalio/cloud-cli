...
func getConfigDir(cctx *CommandContext) string {
	cfgDir := cctx.RootCommand.ConfigDir
	if cfgDir == "" {
...
func loadTokenConfig(cctx *CommandContext) (*TokenConfig, error) {
	tokenConfig := filepath.Join(getConfigDir(cctx), tokenConfigFile)

	_, err := os.Stat(tokenConfig)
	if err != nil {
		if os.IsNotExist(err) {
			cfg, err := login(cctx, nil)
			if err != nil {
	...
	config.cctx = cctx // used for token refreshes.
...
func (c *TokenConfig) Token() (*oauth2.Token, error) {
	...
	token, err := c.OAuthConfig.TokenSource(c.cctx.Context, c.OAuthToken).Token()
	if err != nil {
		...
		if (errors.As(err, &retrieveErr) && retrieveErr.ErrorCode == invalidGrantErr) ||
			...
			cfg, err := login(c.cctx, c)
			if err != nil {
				return nil, fmt.Errorf("failed to login to retrieve new refresh token: %w", err)
			}

			token, err = cfg.OAuthConfig.TokenSource(cfg.cctx.Context, cfg.OAuthToken).Token()
			if err != nil {
...
// GetAPIKey implents the APIKeyReader interface
func (c *TokenConfig) GetAPIKey(ctx context.Context) (string, error) {
	token, err := c.Token()
	if err != nil {
		return "", err
	}
